...
class FeatureOutputProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, train_file):
        ...
        return self._create_examples_from_json(
            self._read_json(os.path.join(data_dir, train_file)), "test")
    
    def get_dev_examples(self, data_dir, dev_file):
        """See base class."""        
        return self._create_examples_from_json(
            self._read_json(os.path.join(data_dir, dev_file)), "test")
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for text_a in bag_text_a: # human-label sen, google sen, baidu sen, xiaoniu sen
            ...
            if '[E1]' not in tokens or '[/E1]' not in tokens or '[E2]' not in tokens or '[/E2]' not in tokens:
                print("NO ENTITY")
                print(tokens)
            # assert '[E1]' in tokens and '[/E1]' in tokens and '[E2]' in tokens and '[/E2]' in tokens
            if '[E1]' in tokens:
        ...
        if ex_index < 2:
            ...
            sources = ["human", "google", "baidu", "xiaoniu"]
            i=0
            #logger.info(source)
